...
1. **Data Management**
   (`src/final_project_aksoltans/data_management/task_data_management_template.py`)

   - Loads raw CSV data, cleans it, saves as pickle to `bld/data/`

1. **Analysis** (`src/final_project_aksoltans/analysis/task_analysis_template.py`)

   - Fits logit models and generates predictions
   - Iterates over `TEMPLATE_GROUPS` defined in `config.py`

1. **Final Outputs** (`src/final_project_aksoltans/final/task_final_template.py`)
...
- `src/final_project_aksoltans/config.py`: Central path definitions (`SRC`, `ROOT`,
  `BLD`, `DOCUMENTS`) and `TEMPLATE_GROUPS` for iterative task generation
...
- `src/final_project_aksoltans/`: Source code (hand-written)
